...
public class SStopwatchActivity extends Activity implements OnClickListener {
	...
	private SparseArray<StopwatchThread> stopwatchThreadList = new SparseArray<StopwatchThread>(
			10);
	...
	public void onClick(View v) {
		...
		case 200:
			...
			logger.log(Level.INFO,
					"Toggle button state:"
							+ laneLayoutList.get(v.getId() - 200)
									.getToggleButton().isChecked());
			if (laneLayoutList.get(v.getId() - 200).getToggleButton()
					.isChecked()) {
				stopwatchThreadList.append(
						v.getId() - 200,
						new StopwatchThread(laneLayoutList.get(v.getId() - 200)
								.getTextViewChronometer(), laneLayoutList.get(
								v.getId() - 200).getScrollFrame(),
								laneLayoutList.get(v.getId() - 200).getId(),
								this));
